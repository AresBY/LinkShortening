...
using LinkShortening.Presentation.Controllers;
...
namespace LinkShortening.Tests
{
    public class UrlControllerTests
    {
        ...
        [Fact]
        public async Task OnCreateUpdateAsync_ReturnsBadRequest_OnInvalidUrl()
        {
            // Arrange
            ...
            // Act
            var result = await _controller.OnCreateUpdateAsync(urlPl) as BadRequestObjectResult;

            // Assert
            Assert.NotNull(result);
            Assert.Equal("Получен некорректный URL: invalid-url", result.Value);
        }
